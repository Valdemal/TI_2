...
template<typename baseType>
matrix::Matrix<baseType>
matrix::Matrix<baseType>::operator+(const matrix::Matrix<baseType> &b) const {
...
matrix::Matrix<baseType>::operator*(const matrix::Matrix<baseType> &b) const {
    if (this->_n == b._m) {
        ...
        throw std::runtime_error("Matrix sizes are not suitable for"
                                 "multiplication!");
}

template<typename baseType>
matrix::Matrix<baseType>
matrix::Matrix<baseType>::operator+(const baseType &val) const {
...
template<typename baseType>
matrix::Matrix<baseType>
matrix::Matrix<baseType>::mulRowVector(const std::vector<baseType> &v) const {
    Matrix row(1, v.size());
...
template<typename baseType>
matrix::Matrix<baseType>
matrix::Matrix<baseType>::mulColVector(const std::vector<baseType> &v) const {
...
template<typename baseType>
void matrix::Matrix<baseType>::
insertColumn(size_t j, const std::vector<baseType> &column) {
    for (size_t i = 0; i < _m; ++i) {
...
template<typename baseType>
void matrix::Matrix<baseType>::
insertRow(size_t i, const std::vector<baseType> &row) {
...
matrix::Matrix<baseType> matrix::Matrix<baseType>::inverse() const {
    if (isSquare()) {
        Matrix temp(*this); // Промежуточная матрица

        auto e = Matrix::E(temp._m); // Еденичная матрица

        // Присоединение еденичной матрицы к промежуточной
        for (size_t i = 0; i < e._m; ++i)
            temp.insertColumn(temp._n, e[i]);

        temp.gaussForward(); // Прямой ход метода Гаусса

        Matrix res(this->_m, this->_n); // Матрица результат

        for (size_t j = 0; j < _n; j++) {
            // Решение j-й СЛАУ
            std::vector<baseType> t = temp.gaussReverseForSystem();

            if (t.empty()) // Если СЛАУ не имеет решений - ошибка
                throw std::runtime_error("Can't find inverse to"
                                         "degenerate matrix");

            // Добавление решения в матрицу-результат
            for (size_t i = 0; i < res._m; ++i)
                res[t.size() - i - 1][t.size() - j - 1] = t[i];

            // Удаление стоблца свободных членов решенной СЛАУ
            temp.eraseColumn(temp.cols() - 1);
        }

        return res;
    } else
        throw std::runtime_error("Can't find inverse matrix to"
                                 "non-square matrix!");
}
...
baseType matrix::Matrix<baseType>::det() const {
    if (this->isSquare()) {
        ...
        throw std::runtime_error("Can't find determinant of"
                                 " non-square matrix!");
}
    ...
    for (size_t k = 0; k < _m; ++k) {
        ...
        if (_values[k][k] != 0) {
            for (size_t i = k + 1; i < _m; ++i) {
                ...
                addVectorMulOnNumToVector(_values[i], _values[k], fi);
//                modifyStr(i, k, fi);
            }
        }
    }
...
template<typename baseType>
bool matrix::Matrix<baseType>::haveEqualSize
        (const matrix::Matrix<baseType> &a, const matrix::Matrix<baseType> &b) {
    return a._m == b._m && a._n == b._n;
}

template<typename baseType>
void matrix::addVectorMulOnNumToVector
        (std::vector<baseType> &modifyV,
         const std::vector<baseType> &refV,
         baseType fi) {

    for (size_t i = 0; i < modifyV.size(); ++i) {
        modifyV[i] += refV[i] * fi;
    }
}